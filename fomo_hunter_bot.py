...
PROMPT_TEMPLATES = {
    # ... (جميع قوالب الأوامر تبقى كما هي، فهي لا تزال فعالة)
    "PRO_SCAN": "...", "MOMENTUM_SCAN": "...", "WHALE_RADAR": "...",
    "GEM_HUNTER": "...", "SNIPER_TRIGGER": "...", "SNIPER_RETEST": "...",
    "DIVERGENCE_SCAN": "...", "STRATEGY_STATS": "...",
}
...
async def get_ai_analysis(report_data: dict, session: aiohttp.ClientSession):
    ...
    payload = {"inputs": final_prompt, "parameters": {"max_new_tokens": 512}}
    ...
    try:
        async with session.post(HF_API_URL, headers=headers, json=payload, timeout=HTTP_TIMEOUT) as response:
            ...
            if isinstance(result, list) and result:
                ...
                # تنظيف الرد لإزالة الأمر الأصلي
                assistant_response = generated_text.split("<|start_header_id|>assistant<|end_header_id|>")[-1].strip()
                return assistant_response if assistant_response else "لم يتمكن الذكاء الاصطناعي من توليد رد."
